...
public class SecondaryDropTarget extends ButtonDropTarget implements OnAlarmListener {
    ...
    protected void setupUi(int action) {
        ...
        if (action == UNINSTALL) {
            setDrawable(com.android.launcher3.res.R.drawable.ic_uninstall_no_shadow);
            updateText(com.android.launcher3.res.R.string.uninstall_drop_target_label);
        } else if (action == DISMISS_PREDICTION) {
            setDrawable(com.android.launcher3.res.R.drawable.ic_block_no_shadow);
            updateText(com.android.launcher3.res.R.string.dismiss_prediction_label);
        } else if (action == RECONFIGURE) {
            setDrawable(com.android.launcher3.res.R.drawable.ic_setting);
            updateText(com.android.launcher3.res.R.string.gadget_setup_text);
        }
    }
    ...
    protected ComponentName performDropAction(View view, ItemInfo info, InstanceId instanceId) {
        ...
        if (mCurrentAccessibilityAction == DISMISS_PREDICTION) {
            if (FeatureFlags.ENABLE_DISMISS_PREDICTION_UNDO.get()) {
                mLauncher.getDragLayer()
                        .announceForAccessibility(getContext().getString(com.android.launcher3.res.R.string.item_removed));
                Snackbar.show(mLauncher, com.android.launcher3.res.R.string.item_removed, com.android.launcher3.res.R.string.undo, () -> { }, () -> {
        ...
        if (cn == null) {
            ...
            Toast.makeText(mLauncher, com.android.launcher3.res.R.string.uninstall_system_app_text, Toast.LENGTH_SHORT).show();
            return null;
        }
        try {
            Intent i = Intent.parseUri(mLauncher.getString(com.android.launcher3.res.R.string.delete_package_intent), 0)
