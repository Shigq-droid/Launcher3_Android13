...
public class ShortcutMenuAccessibilityDelegate extends LauncherAccessibilityDelegate {
    ...
    private static final int DISMISS_NOTIFICATION = com.android.launcher3.res.R.id.action_dismiss_notification;

    public ShortcutMenuAccessibilityDelegate(Launcher launcher) {
        super(launcher);
        mActions.put(DISMISS_NOTIFICATION, new LauncherAction(DISMISS_NOTIFICATION,
                com.android.launcher3.res.R.string.action_dismiss_notification, KeyEvent.KEYCODE_X));
    }
    ...
    protected boolean performAction(View host, ItemInfo item, int action, boolean fromKeyboard) {
        if (action == ADD_TO_WORKSPACE) {
            ...
            announceConfirmation(com.android.launcher3.res.R.string.notification_dismissed);
            return true;
        }
        return false;
    }
