...
public class TaskIconCache implements DisplayInfoChangeListener {
    ...
    private Bitmap getIcon(TaskDescription desc, int userId) {
        if (desc.getInMemoryIcon() != null) {
            return desc.getInMemoryIcon();
        }
        return TaskDescription.loadTaskDescriptionIcon(
    ...
    private BaseIconFactory getIconFactory() {
        if (mIconFactory == null) {
            mIconFactory = new BaseIconFactory(mContext,
                    ...
                    mContext.getResources().getDimensionPixelSize(com.android.launcher3.res.R.dimen.taskbar_icon_size));
        }
        return mIconFactory;
    }
